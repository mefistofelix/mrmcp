...
- `list_commands`;
- `exec`, `exec_start`, `exec_attach`, `exec_write`, `exec_kill`, `exec_list`;
...
Persistent execution is deliberately explicit and label-based. `exec_start` requires a short client-chosen label whose logical identity is `(context_handle, label)`: the same label may exist in different Sessions, but concurrent retained processes in the same Session cannot share it. The server reserves that key across asynchronous startup so concurrent `exec_start` calls cannot race into the same label. `exec_start` starts the process, retains the complete normalized stdout/stderr transcript for the retained process lifetime, retains the client stdin writes internally for diagnostics, keeps stdin open and returns JSON without streaming output. `exec_attach(label)` consumes unread combined output through an internal per-process cursor. With `_meta.progressToken`, it first emits all unread backlog as progress, follows live output until the process exits or is killed, and finally returns the complete unread transcript covered by that attachment. Without a progress token, it is a long-poll/chunk API without public offsets: if unread data already exists it returns immediately with at most 16 KiB; otherwise it waits until data appears, then returns when 16 KiB have accumulated or 100 ms have elapsed after the first new data. The result includes `remaining_bytes`, the UTF-8 byte count already buffered after the returned chunk. Call `exec_attach` again immediately while `remaining_bytes > 0`; when it is `0` and `status` is still `running`, calling again waits for future output or process termination. If the child is killed/fails while attached, the final available output is still returned together with the final `killed`/`failed` status. Disconnecting `exec_attach` only detaches and never kills the persistent process; only one attachment may be active for a label. `exec_write(label)` writes or closes stdin, `exec_kill(label)` terminates the persistent child, and `exec_list` reports retained labels/states. No public polling interval, output offset, process id or PID is part of the MCP control/result contract; internal ids/PIDs remain available only to administration/logging code. Persistent process state does not survive a server restart. `separate_streams=true` on `exec` or `exec_attach` adds complete current stdout/stderr snapshots to the final result while live progress remains the combined observed-order stream. Tool Calls displays the same normalized process output.
