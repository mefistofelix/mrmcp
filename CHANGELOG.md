...
- Confirmed CDP poll/subscription/browser/target state is owned by the CDP subsystem and independent of Tool Call history; Payload-retained Tool Calls remain diagnostic-only for Browser inspection/replay.
- Moved reversible Trash metadata from sibling JSON manifests into normalized SQLite `trash_transactions` / `trash_items` tables (schema v3). Each item caches original path/type and non-directory byte size at trash time (directory size remains unknown) while the Trash UI checks physical payload presence live, explicitly exposing missing-on-disk items without losing metadata; the sidebar badge is now a SQL count rather than a filesystem scan.
...
- Added explicit `schema_meta` / `DB_SCHEMA_VERSION=3` fail-fast validation. There is no migration/backfill path: unversioned or differently-versioned development databases are rejected with instructions to delete `.mrmcp/mrmcp.sqlite` and restart cleanly.
...
- Reworked Tool Call persistence around smaller critical-path writes: cached configuration/server/tool descriptors, a bounded prepared-statement LRU, 5-second SQLite busy timeout, transactional log start, deduplicated historical descriptors, direct `progress_requested` metadata, bounded deferred binary/MCP diagnostics, incremental deferred FTS indexing, and no redundant structured-result copy in `stdout`.
- Added independent Tool Call history retention by storage tier: Disk uses whole hours and Memory uses minutes; `0` keeps Disk indefinitely or Memory until restart. Positive values lazily remove eligible completed history without resetting Tool Call ids or deleting the origin of a still-running execution.
- Made `tools_log` compact by default with `detail=summary`, exact-id inspection, explicit `payload_mode`/availability fields and stateless `before_id` pagination. Browser and Automation replay/history details now use only Full-retained Tool Calls, so bounded or metadata-only logs are never mistaken for complete replay sources.
- Optimized filesystem traversal without implicit dependency/build exclusions: provable include prefixes prune impossible subtrees, ambiguous/global globs fail open, metadata work is bounded within each directory, and `fs_grep`/`fs_trash` use lightweight discovery where rich glob metadata is unnecessary.
- Made `fs_glob` lightweight by default with `metadata=false`, a 1000-entry default page and explicit stateless `next_after_path`; metadata remains opt-in. Added a 1 MiB response budget to `fs_grep` with explicit truncation reason/cursor and a 2 MiB aggregate rolling budget to `fs_read`, while preserving per-file stateless continuation and allowing larger optional limits when needed.
- Reduced common event-loop/SQLite work by batching HTTP request metrics, caching hot aggregates/Session stats, avoiding repeated descriptor/schema construction, using structural-sharing result redaction/direct log serialization, and distributing FTS work across event-loop turns instead of draining bursts synchronously.
- Simplified the clean current SQLite schema with no compatibility migrations or backfills. Development databases using older incompatible Tool Call/process table shapes must be recreated rather than silently altered.
- Stopped building an unbounded pretty-JSON copy before clipping large structured MCP text results: rendering now stops at the 1 MiB text budget while leaving complete `structuredContent` untouched. The same bounded serializer is used for Light Tool Call JSON previews, avoiding repeated full traversal of very large results.
- Added a dedicated **Trash** sidebar section backed directly by `.mrmcp/trash` manifests/payloads, with a live item-count badge, transaction/item inventory, per-item or whole-transaction restore/delete, orphan visibility and global Empty Trash. Removed Trash/Untrash activity from Dashboard so functional trash state no longer depends on Tool Call history.
...
- Upgraded expanded Tool Call inspection with one vendored JSONEditor 10.4.3 minimalist tree viewer for input, raw return, final MCP result and descriptor schemas. Detectable Base64/data-URL/MCP image, audio and blob content from tool input/output is retained separately in SQLite with direction, JSON path, MIME and size; image content gets compact thumbnails with hover enlargement while non-image binary remains metadata-only. The same JSONEditor runs in editable tree mode for JSON Memory values, with node-level editing synchronized through the normal Deno-owned form/submit path; text Memory remains a plain textarea.
- Removed MrMCP's direct Sharp screenshot path. `cdp_call` WebP screenshot post-processing now uses the public `auto.vips.decodeImage` / `auto.vips.encodeImage` API exposed by current Auto.js instead of importing its internal `vips.js`; original screenshots load no codec layer, scale remains configurable, and the current Auto.js WebP encoder exposes fixed Q=80.
- Added one generic `telegram_req` Bot API tool using native `fetch()`. The local user configures only the Bot token in Settings; agents supply Bot API method + JSON payload without receiving or handling the token. Numeric-string chat ids are normalized, runtime `migrate_to_chat_id` redirects are remembered and retried once, and no TDLib/Telegram SDK or `doc/` code is imported.
- Memory values now explicitly choose JSON or text. `memory_set` requires `json=true|false` when setting; JSON text is validated before storage, text is preserved verbatim, results expose the type flag, and the Memory GUI labels/edits JSON vs TEXT alongside existing TTL/scope/date filters.

## 0.10.116

- Added a dependency-free low-level CDP surface: `cdp_call`, `cdp_subs`, and `cdp_poll`. `cdp_call` is always an ordered `calls[]` batch (including one-call batches), may mix browsers and logical page targets, applies one batch-wide `wait` policy and keeps standard CDP `{method,params}` untouched. A namespaced `call._mrmcp` extension adds the reference augmented-XPath `click` and `find` behaviors without separate MCP tools or protocol-method collisions.
- Added persistent global browser/profile labels under `.mrmcp/cdp/<browser>/` with deterministic/probed unique loopback ports stored in SQLite. The first call reconnects to a still-running browser or launches a compatible Chromium browser automatically, while logical page target labels persist only their CDP targetId and lazily recreate missing pages; sessionId mappings stay runtime-only and are rebuilt through CDP after reconnect.
- Preserved the low-level behavior from `doc/CDP.js` without importing it: tuned browser launch behavior, auto-attach/discovery, paused new targets, Runtime/Page/Network/ServiceWorker initialization, focus emulation, `_send_to_cdp` through `Runtime.addBinding`, push-messaging BackgroundService observation, augmented XPath `ends-with`/`icontains`, retrying click and compact XPath find. Page initialization explicitly sends `Runtime.disable` while the target is still paused and only then resumes it with `Runtime.runIfWaitingForDebugger`.
- Added one bounded per-browser CDP ring (10,000 messages / 32 MiB) plus unified `cdp_subs`: one call adds/removes subscriptions, `*` is supported for catch-all add/all removal and for target/method matching, and optional regex filters run over the complete serialized raw CDP message for diagnostic/content matching. Subscriptions keep ascending cursors; ad-hoc polling retains exact/prefix method, target/type/id filters, dropped-message accounting and reconnect stream resets. Responses remain retained independently, including `wait=false` retrieval by id.
- Added optional `cdp_call` screenshot post-processing for standard `Page.captureScreenshot`: all normal CDP screenshot params remain available, while outer `_image` can explicitly keep Base64 output as-is or lazily convert/resize it to WebP with quality/scale through the existing cached Sharp loader. Screenshots remain response data and are not implicitly written to disk.
- Added explicit persistent key-value Memory through `memory_find` and `memory_set`. Every call chooses Session or named-Workspace scope; values are arbitrary JSON up to 1 MiB, optional TTL expires lazily, exact lookup is simply `memory_find(key=...)`, replacements receive a fresh id/set timestamp, and Session/Workspace deletion removes owned memories while Clear Operational Data preserves them. The desktop Memory page filters by scope, Session, Workspace, set-date range and text and supports full JSON/TTL inspection, editing and deletion.
- Folded agent-side Workspace creation into `open_workspace(create=true)` and removed the public `create_workspace` tool. Missing Workspaces are never created implicitly. `open_workspace` now also returns whether this call created the Workspace plus compact Workspace/Session Memory counts and up to five latest keys for orientation.
