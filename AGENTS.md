...
- `.github/workflows/test-macos.yml` — separate native macOS GUI smoke test. Keep both Intel (`macos-15-intel`) and Apple Silicon (`macos-15`) jobs; each must compile for its runner architecture, launch the standalone binary under the runner's real macOS GUI session, wait beyond the 10-second MrMCP WebView bootstrap deadline, and fail with the captured log if the process exits or reports the bootstrap timeout.
