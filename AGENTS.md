...
MrMCP 0.10.90 consists of four root project files plus one versioned asset directory:
...
- HTTP debug: recording setting, filtered stored rows and at most one selected detail row. Turning recording off must only stop new `debug_logs` inserts; existing rows remain queryable, filterable and expandable until explicitly cleared.
