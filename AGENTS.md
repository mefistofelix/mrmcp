...
MrMCP 0.10.93 consists of four root project files plus one versioned asset directory:
...
Use only the direct Deno import `import { Tauriless } from "npm:@mefistofelix/tauriless@0.1.11";` for the desktop shell. Keep the version pinned because Windows startup and notification identity depend on Tauriless' pre-WebView `tauriless:set-app-user-model-id` bridge.
...
- Dashboard: endpoint summary and aggregates, Trash/Untrash activity, plus the compact active/recent Tool Call table. That table includes every in-flight Tool Call and calls completed within the last three seconds, using the same bounded tool/argument preview, Session id, elapsed time and persisted `progress_requested` marker. Deno schedules one-shot renders while calls are active and at completed-row TTL expiry; CSS may fade a completed row, but the browser must not own the timer/countdown or poll.
...
- Persistent execution uses `exec_start`, `exec_attach`, `exec_write`, `exec_list` and `exec_kill`. `exec_start` requires a client-chosen label matching `^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$`. Its logical key is exactly `(context_handle, label)`: different Sessions may reuse the same label, while the same Session may not have two retained processes with that label. Reserve that key before asynchronous startup work and release the reservation in `finally` so concurrent starts cannot race past the uniqueness check. It starts immediately, retains the complete normalized stdout/stderr transcript for the retained process lifetime, keeps stdin open, records client stdin writes internally and returns JSON without streaming. A finished retained record may be replaced when the same label is reused.
- `exec_attach(label)` consumes unread combined output through an internal cursor and exposes no public offset. With `_meta.progressToken`, emit all unread backlog through progress, continue emitting live output until the child exits/kills, then return in the final result the entire unread transcript covered by that attachment with `remaining_bytes: 0`. Without `_meta.progressToken`, if unread data exists return immediately with at most 16 KiB; otherwise long-poll until output arrives, then return once 16 KiB are available or 100 ms have elapsed after the first new data. `remaining_bytes` is the exact UTF-8 byte count already buffered after the returned chunk. Agents should immediately call again while it is nonzero; with zero plus `status: running`, another call is still valid and waits for future output/termination. If the process kills/fails while attached, preserve and return the last available output together with the final process status. Only one attachment may be active per label. Disconnecting an attachment detaches only and never terminates the persistent child. `separate_streams: true` augments the final result with complete current stdout/stderr snapshots; live progress stays combined.
- `exec_write(label)` writes/closes stdin, `exec_kill(label)` terminates only persistent children, and `exec_list` exposes retained Session-scoped labels/state. Do not expose `process_id` or OS PID in MCP process inputs or results; both may remain in internal database/GUI/diagnostic projections. Persistent process state/transcripts are in-memory and do not survive server restart.
...
25. Confirm the desktop shell uses only `import { Tauriless } from "npm:@mefistofelix/tauriless@0.1.11";`, with no hand-written FFI wrapper, vendored Tauriless native binaries, Rust/Tauri scaffold, Neutralinojs, npm project files or CLI files.
...
35. Verify `exec`/`exec_start` schemas describe argv as verbatim ordered arguments and process normalization happens before buffering/streaming, removes ANSI/OSC/control sequences without erasing prior progress states, and converts standalone `\r` to a line break. Confirm foreground `exec`/custom commands without progress return ordinary final JSON, while `_meta.progressToken` + SSE produces 16 KiB/100 ms progress and the final result still contains the complete transcript; disconnecting foreground work kills the child. Confirm `exec_start(label)` returns immediately without streaming and retains the complete process transcript. Verify `exec_attach(label)` with progress emits unread backlog + live output to process end and then returns the whole unread transcript with `remaining_bytes=0`; without progress it returns immediately when backlog exists, otherwise waits for data, returns at most 16 KiB after the 16 KiB/100 ms threshold, reports exact `remaining_bytes`, advances its internal cursor, and can be called again with `remaining_bytes=0,status=running` to wait for future output. Killing/failing the child while attached must wake the call and return the last output plus final status. Disconnecting attach only detaches. `exec_write`/`exec_kill` use label, and no `exec_poll`, public offsets, `wait_ms`, public process id or public PID remain. Verify persistent uniqueness is `(context_handle,label)`, the same label works in different Sessions, a same-Session concurrent duplicate cannot race through asynchronous startup, completed labels may be reused, and stdin writes are retained internally. `separate_streams: true` augments only final process results. On Windows, verify managed children use `node:child_process.spawn(..., { windowsHide: true })`, completion observes the child `exit` rather than waiting indefinitely for inherited pipe closure, and termination uses only the managed child runtime `kill()` API with no `taskkill`/platform helper or false process-tree guarantee.
...
45. Verify the Dashboard Trash card derives its count and latest action/path from actual `.mrmcp/trash/<action_id>` directories across the program folder and every configured Workspace, so Empty Trash immediately produces zero with no stale path detail even when historical `trash_paths` Tool Call logs remain. Untrash count/details remain historical Tool Call activity and its path is labeled historical. Verify the Dashboard Active Tool Calls table contains all in-flight calls plus only the previous three seconds of completed calls, uses the shared compact summary/Session/time/progress marker, fades only as presentation, and is removed/refreshed by Deno-owned one-shot timers with no browser polling/countdown. Verify Settings Clear Operational Data and Dashboard Empty Trash use the shared Promise maintenance barrier: in-flight Tool Calls finish, new calls wait, maintenance runs, then waiting calls continue. Confirmation dialogs close immediately, no completion dialog appears, only the active action button shows spinner/live in-flight+waiting counts, and Dashboard Tool Calls In Flight tracks `activeCallControls.size` without browser polling.
