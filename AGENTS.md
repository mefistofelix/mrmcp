...
MrMCP 0.10.86 consists of four root project files plus one versioned asset directory:
...
Use only the direct Deno import `import { Tauriless } from "npm:@mefistofelix/tauriless@0.1.7";` for the desktop shell. Keep the version pinned because Windows startup depends on the pre-WebView `tauriless:set-app-user-model-id` bridge added in Tauriless 0.1.7.
...
25. Confirm the desktop shell uses only `import { Tauriless } from "npm:@mefistofelix/tauriless@0.1.7";`, with no hand-written FFI wrapper, vendored Tauriless native binaries, Rust/Tauri scaffold, Neutralinojs, npm project files or CLI files.
