...
MrMCP 0.10.87 consists of four root project files plus one versioned asset directory:
...
- `open_workspace(name, current_context_handle?)` requires an enabled named Workspace. With an active current handle it reassigns that same Session immediately and preserves the handle; otherwise it creates a new Session attached to the Workspace.
...
- Tool/server descriptions must direct the agent to use the Workspace identity/guidance fields returned directly by `open_workspace` and read and follow `agent_guidance_path` when non-null.
