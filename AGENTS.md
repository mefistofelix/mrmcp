...
MrMCP 0.10.100 consists of four root project files, two GitHub Actions workflows and one versioned asset directory:
...
- `.github/workflows/test-macos.yml` — separate native macOS GUI smoke test. It runs on relevant pushes to `main`, pull requests and manual dispatch. Keep both Intel (`macos-15-intel`) and Apple Silicon (`macos-15`) jobs; each must compile for its runner architecture, launch the standalone binary under the runner's real macOS GUI session, wait beyond the 10-second MrMCP WebView bootstrap deadline, and fail with the captured log if the process exits or reports the bootstrap timeout.
