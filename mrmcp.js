/*
MrMCP 0.10.43 — Deno-owned event-driven UI and Stateless MCP server with explicit server opaque values and an embedded WebView desktop window.
...
const SERVER_OPAQUE_TTL_MS = 30 * 24 * 60 * 60 * 1000;
const SERVER_OPAQUE_INPUT_DESCRIPTION = "Opaque value generated by this server. After the server issues it, pass this exact value unchanged in every later tool call to this server. Omit it only before the server has issued a value, or when a server response explicitly instructs you to omit it. Never invent, modify, shorten, derive, or substitute a value.";
const SERVER_OPAQUE_OUTPUT_DESCRIPTION = "Opaque value generated by this server. Copy this exact value into server_opaque on every later tool call to this server. Never invent or modify it.";
const SERVER_OPAQUE_RULE = "Server opaque rule: omit server_opaque only before this server has issued a value, or when a server response explicitly instructs you to omit it. The server will return a value and ask you to retry without executing the requested operation. After receiving it, pass it back unchanged in every later tool call to this server.";
...
async function backend() {
  ...
  const uiState = {
    ...
    logs: { page: 1, query: "", opaque: "", status: "", pageSize: 25, openRowId: "", selfTest: null },
  ...
  function uiScopesForSql(sql) {
    ...
    if (/\bserver_opaques\b/.test(statement)) ["sessions", "logs", "dashboard"].forEach(scope => scopes.add(scope));
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS logs(
      ...
      duration_ms INTEGER,
      server_opaque TEXT NOT NULL DEFAULT ''
    );
    ...
    CREATE INDEX IF NOT EXISTS logs_opaque ON logs(server_opaque,started_at DESC);
    ...
    CREATE TABLE IF NOT EXISTS server_opaques(
      id TEXT PRIMARY KEY,
      server_id INTEGER NOT NULL,
    ...
    CREATE INDEX IF NOT EXISTS server_opaque_owner ON server_opaques(server_id,owner_key,updated_at DESC);
    CREATE INDEX IF NOT EXISTS server_opaque_active ON server_opaques(last_active_at DESC);
  ...
  const requiredSchema = {
    ...
    server_opaques: ["id", "owner_key", "default_root", "created_at", "updated_at", "last_active_at", "protocol_version"],
    process_runs: ["owner_key", "server_opaque", "stdout_tail", "stderr_tail"],
    logs: ["id", "server_name", "tool", "status", "input_json", "server_opaque"],
...
(function () {
  ...
  // Explicit server opaque values carry application state over the stateless MCP transport.
  // An empty root selection falls back to APP_DIR. Values are owner-scoped and random.
  ...
  function opaqueById(p, ownerKey, id) {
    return id ? one(
      "SELECT * FROM server_opaques WHERE id=? AND server_id=? AND owner_key=?",
      id, p.id, ownerKey,
    ) : null;
  }
  function createOpaqueRecord(p, ownerKey, id, label, rootName = "", protocolVersion = "") {
    const root = rootName ? serverRoots(p).find(item => item.name === rootName) : null;
    const now = Date.now();
    run(`INSERT INTO server_opaques(id,server_id,owner_key,label,pinned_roots_json,
      default_root,created_at,updated_at,last_active_at,protocol_version) VALUES(?,?,?,?,?,?,?,?,?,?)`,
      id, p.id, ownerKey, String(label || "server opaque"), JSON.stringify(root ? [root.name] : []),
      root?.name || "", now, now, now, String(protocolVersion || ""));
    return one("SELECT * FROM server_opaques WHERE id=?", id);
  }
  function createServerOpaque(p, ownerKey, protocolVersion = "") {
    let id;
    do id = `opaque_${randomToken(24)}`;
    while (one("SELECT 1 FROM server_opaques WHERE id=?", id));
    return createOpaqueRecord(p, ownerKey, id, "server opaque", "", protocolVersion);
  }
  const serverOpaqueExpired = ws => !!ws && Date.now() - Number(ws.last_active_at || ws.created_at || 0) > SERVER_OPAQUE_TTL_MS;
  function resolveServerOpaque(p, ownerKey, suppliedId = "", protocolVersion = "") {
    const id = String(suppliedId ?? "");
    if (!id) {
      const opaque = createServerOpaque(p, ownerKey, protocolVersion);
      return { kind: "bootstrap", record: opaque, server_opaque_status: "active", retry_required: true };
    }
    if (id.length > 256) return { kind: "invalid", record: null, supplied_id: id, server_opaque_status: "invalid", retry_required: true };
    const opaque = opaqueById(p, ownerKey, id);
    if (!opaque) return { kind: "invalid", record: null, supplied_id: id, server_opaque_status: "invalid", retry_required: true };
    if (serverOpaqueExpired(opaque)) return { kind: "expired", record: opaque, server_opaque_status: "expired", retry_required: true };
    const now = Date.now(), observed = String(protocolVersion || opaque.protocol_version || "");
    run("UPDATE server_opaques SET last_active_at=?,protocol_version=? WHERE id=?", now, observed, opaque.id);
    return { kind: "active", record: one("SELECT * FROM server_opaques WHERE id=?", opaque.id), server_opaque_status: "active", retry_required: false };
  }
  function getOpaqueRecord(p, ownerKey, id = "") {
    const ws = opaqueById(p, ownerKey, id);
    if (!ws) throw new Error("The supplied server_opaque value is unknown or belongs to another authenticated client");
    if (serverOpaqueExpired(ws)) throw new Error("The server_opaque value has expired");
    return ws;
  }
  function selectedOpaqueRoot(p, ws) {
    const roots = serverRoots(p);
    const configured = roots.find(item => item.name === ws.default_root) || null;
    if (ws.default_root && !configured) run(
      "UPDATE server_opaques SET pinned_roots_json='[]',default_root='',updated_at=? WHERE id=?",
      Date.now(), ws.id,
    );
    return configured || fallbackWorkspaceRoot(p);
  }
  function opaqueSnapshot(p, ws) {
    const roots = serverRoots(p), root = selectedOpaqueRoot(p, ws);
    return {
      server_opaque: ws.id,
      kind: "server_opaque",
      label: ws.label || ws.id,
      server_opaque_status: serverOpaqueExpired(ws) ? "expired" : "active",
      protocol_version: ws.protocol_version || "unknown",
      root: { name: root.name, path: root.path, fallback: !!root.fallback },
      created_at: ws.created_at,
      updated_at: ws.updated_at,
      last_active_at: ws.last_active_at,
      expires_at: Number(ws.last_active_at || ws.created_at || 0) + SERVER_OPAQUE_TTL_MS,
      available_roots: roots.map(item => ({ name: item.name, path: item.path, selected: item.name === ws.default_root })),
      fallback_root: { name: "mrmcp.js folder", path: APP_DIR, selected: !ws.default_root },
      path_format: "normal path relative to the selected root",
    };
  }
  function selectWorkspaceRoot(p, ws, name) {
    name = String(name || "");
    const root = name ? serverRoots(p).find(item => item.name === name) : null;
    if (name && !root) throw new Error(`Unknown or disabled root: ${name}`);
    run("UPDATE server_opaques SET pinned_roots_json=?,default_root=?,updated_at=? WHERE id=?",
      JSON.stringify(root ? [root.name] : []), root?.name || "", Date.now(), ws.id);
    return opaqueSnapshot(p, getOpaqueRecord(p, ws.owner_key, ws.id));
  }
  function workspaceAction(p, ownerKey, args) {
    const action = String(args.action || "status"), roots = serverRoots(p);
    if (action === "list_roots") return {
      roots: roots.map(root => ({ name: root.name, path: root.path })),
      fallback_root: { name: "mrmcp.js folder", path: APP_DIR },
      path_format: "normal path relative to the selected root",
    };
    const ws = getOpaqueRecord(p, ownerKey, String(args.server_opaque || ""));
    if (action === "status") return opaqueSnapshot(p, ws);
    if (action === "select") return selectWorkspaceRoot(p, ws, args.root);
    throw new Error("workspace.action must be list_roots, status or select");
  }
  function selectedRoot(p, ownerKey, args = {}) {
    const ws = getOpaqueRecord(p, ownerKey, String(args.server_opaque || ""));
    return { ws: { ...ws, server_opaque: ws.id }, root: selectedOpaqueRoot(p, ws) };
  }
  async function resolveWorkspacePath(p, ownerKey, args, path = ".") {
  ...
  function serverTools(p, fullAccess = true) {
    ...
    const opaqueInput = {
      server_opaque: { type: "string", description: SERVER_OPAQUE_INPUT_DESCRIPTION },
    };
    const opaqueOutputSchema = {
      "$schema": "https://json-schema.org/draft/2020-12/schema",
      type: "object",
      additionalProperties: true,
      properties: {
        server_opaque: { type: "string", description: SERVER_OPAQUE_OUTPUT_DESCRIPTION },
        server_opaque_status: { type: "string", enum: ["active", "invalid", "expired"], description: "State of the supplied or newly issued server_opaque value." },
        retry_required: { type: "boolean", description: "True when the requested operation was not executed and must be retried after following message." },
        message: { type: "string", description: "Server guidance for reusing or recovering the opaque value." },
        execution_log_id: { type: "integer" },
      },
      required: ["server_opaque", "server_opaque_status", "retry_required", "message"],
    };
    const inputEncoding = { type: "string", enum: ["auto", "utf-8", "utf-16le", "utf-16be", "windows-1252", "latin1"], default: "auto" };
    const outputText = {
      output_encoding: { type: "string", enum: ["preserve", "utf-8", "utf-16le", "utf-16be", "windows-1252", "latin1"], default: "preserve" },
    ...
    const defs = {
      workspace: ["List roots, inspect the state associated with the supplied server_opaque value, or select one named root. An empty root selects the folder containing mrmcp.js as the fallback.", {
        properties: {
          action: { type: "string", enum: ["list_roots", "status", "select"], default: "status" },
          root: { type: "string" }, ...opaqueInput,
        },
      }],
      read_file: ["Read a text file with encoding detection and line-ending metadata. Use this instead of js/exec for ordinary source inspection.", { properties: { path: { type: "string" }, encoding: inputEncoding, start_line: { type: "integer", minimum: 1 }, end_line: { type: "integer", minimum: 1 }, ...opaqueInput }, required: ["path"] }],
      read_files: ["Read several text files with encoding detection. Use this instead of js/exec for ordinary source inspection.", { properties: { paths: { type: "array", minItems: 1, maxItems: 100, items: { type: "string" } }, encoding: inputEncoding, max_bytes_per_file: { type: "integer", minimum: 1, maximum: 5242880, default: 1048576 }, ...opaqueInput }, required: ["paths"] }],
      write_file: ["Create or overwrite one text file while preserving or explicitly setting encoding, BOM and line endings. Prefer this over js/exec when supplying complete text.", { properties: { path: { type: "string" }, content: { type: "string" }, create_parents: { type: "boolean", default: true }, expected_sha256: { type: "string" }, ...outputText, ...opaqueInput }, required: ["path", "content"] }],
      write_files: ["Validate and write several text files with rollback, preserving encoding, BOM and line endings by default. Prefer this over scripting for complete-file updates.", { properties: { files: { type: "array", minItems: 1, maxItems: 100, items: { type: "object", additionalProperties: false, properties: { path: { type: "string" }, content: { type: "string" }, expected_sha256: { type: "string" }, ...outputText }, required: ["path", "content"] } }, create_parents: { type: "boolean", default: true }, ...opaqueInput }, required: ["files"] }],
      edit_file: ["Replace exact text in one existing text file. Prefer this over js or exec for targeted edits; match count is validated and encoding, BOM and line endings are preserved by default.", { properties: { path: { type: "string" }, old_text: { type: "string" }, new_text: { type: "string" }, expected_occurrences: { type: "integer", minimum: 1, default: 1 }, input_encoding: inputEncoding, ...outputText, ...opaqueInput }, required: ["path", "old_text", "new_text"] }],
      edit_files: ["Validate and atomically apply exact replacements across several existing text files. This is the preferred tool for multi-file source edits; use js/exec only when the transformation genuinely requires programmatic parsing or cannot be expressed as exact/bulk replacements.", { properties: { edits: { type: "array", minItems: 1, maxItems: 200, items: { type: "object", additionalProperties: false, properties: { path: { type: "string" }, old_text: { type: "string" }, new_text: { type: "string" }, expected_occurrences: { type: "integer", minimum: 1, default: 1 }, input_encoding: inputEncoding, ...outputText }, required: ["path", "old_text", "new_text"] } }, ...opaqueInput }, required: ["edits"] }],
      replace_files: ["Preview or apply a bulk literal/regex replacement while preserving encoding, BOM and line endings by default. Prefer this over js/exec for repeated textual changes.", { properties: { query: { type: "string" }, replacement: { type: "string" }, path: { type: "string", default: "." }, pattern: { type: "string", default: "**/*" }, regex: { type: "boolean" }, case_sensitive: { type: "boolean", default: true }, dry_run: { type: "boolean", default: true }, max_files: { type: "integer", minimum: 1, maximum: 1000, default: 200 }, input_encoding: inputEncoding, ...outputText, ...opaqueInput }, required: ["query", "replacement"] }],
      list_files: ["Glob files recursively under the selected root", { properties: { path: { type: "string", default: "." }, pattern: { type: "string", default: "**/*" }, include_hidden: { type: "boolean" }, include_dependencies: { type: "boolean" }, limit: { type: "integer", minimum: 1, maximum: 10000 }, ...opaqueInput } }],
      search_files: ["Grep text or a regular expression under the selected root", { properties: { query: { type: "string" }, path: { type: "string", default: "." }, pattern: { type: "string", default: "**/*" }, regex: { type: "boolean" }, case_sensitive: { type: "boolean" }, max_results: { type: "integer", minimum: 1, maximum: 2000 }, ...opaqueInput }, required: ["query"] }],
      file_info: ["Return file or directory metadata", { properties: { path: { type: "string" }, ...opaqueInput }, required: ["path"] }],
      create_directory: ["Create a directory and its parents", { properties: { path: { type: "string" }, ...opaqueInput }, required: ["path"] }],
      copy_path: ["Copy a file or directory recursively", { properties: { from: { type: "string" }, to: { type: "string" }, ...opaqueInput }, required: ["from", "to"] }],
      move_path: ["Move or rename a path", { properties: { from: { type: "string" }, to: { type: "string" }, ...opaqueInput }, required: ["from", "to"] }],
      delete_path: ["Delete a file or directory", { properties: { path: { type: "string" }, recursive: { type: "boolean" }, ...opaqueInput }, required: ["path"] }],
      publish_file: ["Publish an existing workspace file. In MCP Apps-capable hosts, the attached minimal preview UI displays images only from the HTTPS resource_link URL; it never embeds, reconstructs, or falls back to Base64. Use this after exec, JavaScript, or another tool creates a screenshot, chart or diagram. The default return_mode is link and is the preferred mode for the preview. inline remains available for clients that specifically need native MCP ImageContent, but it does not provide a URL and therefore cannot drive this preview. both preserves both standard forms when explicitly requested. Temporary links default to 24 hours so the preview can survive ordinary chat navigation, but remain memory-only and are invalidated by a MrMCP restart. Never read binary files as UTF-8 or manually Base64-encode them. No resize, recompression, transcoding or other image optimization is performed.", {
        properties: {
      ...
      exec_poll: ["Read incremental stdout, stderr and status for a managed process", { properties: { process_id: { type: "string" }, stdout_offset: { type: "integer", minimum: 0, default: 0 }, stderr_offset: { type: "integer", minimum: 0, default: 0 }, wait_ms: { type: "integer", minimum: 0, maximum: 30000, default: 0 }, ...opaqueInput }, required: ["process_id"] }],
      exec_write: ["Write text or Base64 to process stdin, or close stdin", { properties: { process_id: { type: "string" }, data: { type: "string", default: "" }, encoding: { type: "string", enum: ["text", "base64"], default: "text" }, close: { type: "boolean", default: false }, ...opaqueInput }, required: ["process_id"] }],
      exec_kill: ["Terminate or force-kill a managed process", { properties: { process_id: { type: "string" }, signal: { type: "string", enum: ["SIGTERM", "SIGKILL"], default: "SIGTERM" }, ...opaqueInput }, required: ["process_id"] }],
      exec_list: ["List active and recent processes belonging to the supplied server_opaque value", { properties: { include_completed: { type: "boolean", default: true }, limit: { type: "integer", minimum: 1, maximum: 200, default: 50 }, ...opaqueInput } }],
    ...
    const tools = [...available].filter(name => defs[name]).map(name => ({
      ...
      description: `${defs[name][0]} ${SERVER_OPAQUE_RULE}`, inputSchema: schema(defs[name][1]),
      outputSchema: opaqueOutputSchema, annotations: annotations(name),
      ...(name === "publish_file" ? {
        _meta: {
    ...
    for (const c of all("SELECT * FROM custom_tools WHERE server_id=? ORDER BY name", p.id)) tools.push({
      name: c.name,
      title: c.name.replaceAll("_", " ").replace(/\b\w/g, x => x.toUpperCase()),
      description: `${c.description || `Run configured command: ${c.command}`} ${SERVER_OPAQUE_RULE}`,
      inputSchema: schema({ properties: {
        ...
        timeout_ms: { type: "integer", minimum: 1, maximum: 3600000, default: 120000 }, ...opaqueInput,
      } }),
  ...
  function validateToolDescriptor(tool) {
    ...
    else if (tool.outputSchema.type !== "object") errors.push("outputSchema.type must be object");
    else for (const key of ["server_opaque", "server_opaque_status", "retry_required", "message"])
      if (!tool.outputSchema.required?.includes(key)) errors.push(`outputSchema missing required ${key}`);
    if (!tool.inputSchema?.properties?.server_opaque)
      errors.push("inputSchema missing server_opaque");
    return errors;
  }
  ...
  function beginLog(p, tool, args, sessionId = "") {
    const inserted = run(`INSERT INTO logs(started_at,server_id,server_name,tool,status,input_json,server_opaque)
      VALUES(?,?,?,?,'received',?,?)`, Date.now(), p.id, "mcp", tool, JSON.stringify(args), String(sessionId || ""));
  ...
  function rejectToolCall(p, tool, args, message, sessionId = "") {
    const id = beginLog(p, tool, args, sessionId), completed = Date.now();
    updateLog(id, {
  ...
  function processView(rec, stdoutOffset = 0, stderrOffset = 0) {
    ...
    return {
      ...
      cwd: rec.cwd_display, server_opaque: rec.server_opaque,
  ...
  async function startManagedProcess(p, ownerKey, args, background, execution = {}) {
    ...
    const rec = {
      id: `proc_${randomToken(18)}`, pid: child.pid, child, owner_key: ownerKey, log_id: Number(execution.logId || 0),
      server_id: p.id, server_name: "mcp", server_opaque: target.ws.id,
      display: spec.display, command_json: JSON.stringify({
        ...
        system_path_inherited: includeSystemPath,
      }),
    ...
    run(`INSERT INTO process_runs(id,pid,server_id,server_name,server_opaque,owner_key,
      command_json,cwd,status,started_at,timeout_ms) VALUES(?,?,?,?,?,?,?,?,?,?,?)`,
      rec.id, rec.pid, p.id, "mcp", rec.server_opaque, ownerKey, rec.command_json,
  ...
  function ownedProcess(id, ownerKey, opaqueValue = "") {
    const rec = processes.get(String(id || ""));
    if (!rec || rec.owner_key !== ownerKey || rec.server_opaque !== String(opaqueValue || ""))
      throw new Error("Unknown process_id for this server_opaque value");
    return rec;
  }
  ...
  function recentProcesses(ownerKey, opaqueValue, includeCompleted = true, limit = 50) {
    const active = [...processes.values()].filter(r => r.owner_key === ownerKey &&
      r.server_opaque === String(opaqueValue || "") && (includeCompleted || r.status === "running"));
    const ids = new Set(active.map(r => r.id));
    const historic = includeCompleted ? all(`SELECT * FROM process_runs WHERE owner_key=? AND server_opaque=? ORDER BY started_at DESC LIMIT ?`, ownerKey, String(opaqueValue || ""), limit)
      .filter(r => !ids.has(r.id)).map(r => ({
        process_id: r.id, pid: r.pid, status: r.status, command: parseJson(r.command_json, r.command_json),
        cwd: r.cwd, server_opaque: r.server_opaque, started_at: new Date(r.started_at).toISOString(),
  ...
  function jsKernelKey(p, ownerKey, opaqueValue) {
    return `${p.id}:${ownerKey}:${opaqueValue}`;
  }
  ...
  async function executeTool(p, name, args, execution) {
      ...
      for (const item of args.paths || []) {
        try {
          ...
          if (st.size > max) files.push({ path: target.display, error: `File exceeds ${max} bytes`, size: st.size });
            ...
            const document = await readTextDocument(target.path, args.encoding || "auto");
            files.push({ path: target.display, content: document.text, size: st.size,
              encoding: document.encoding, bom: document.bom, line_endings: document.line_endings });
          }
    ...
    if (name === "write_file") {
      ...
      let source = null;
      try { source = await readTextDocument(target.path, "auto"); } catch (e) { if (!(e instanceof Deno.errors.NotFound)) throw e; }
    ...
    if (name === "write_files") {
      ...
      try {
        ...
        for (const change of changes) {
          try { change.before == null ? await Deno.remove(change.target.path) : await Deno.writeFile(change.target.path, change.before); } catch {}
        }
        throw e;
      }
    ...
    if (name === "edit_file" || name === "edit_files") {
      ...
      for (const edit of edits) {
        const target = await resolveWorkspacePath(p, ownerKey, args, edit.path);
        const document = await readTextDocument(target.path, edit.input_encoding || "auto");
        const oldText = editNeedle(edit.old_text, document), newText = editNeedle(edit.new_text, document);
        const count = document.text.split(oldText).length - 1, expected = Number(edit.expected_occurrences ?? 1);
        if (count !== expected) throw new Error(`${edit.path}: expected ${expected} occurrences, found ${count}`);
        const output = encodeTextDocument(document.text.split(oldText).join(newText), document, edit);
        changes.push({ target, before: document.bytes, output, count });
      }
      try { for (const change of changes) await Deno.writeFile(change.target.path, change.output.bytes); }
      catch (e) { for (const change of changes) await Deno.writeFile(change.target.path, change.before).catch(() => {}); throw e; }
      return { files: changes.map(change => ({ path: change.target.display, replacements: change.count,
        encoding: change.output.encoding, bom: change.output.bom, line_endings: change.output.line_endings })) };
    }
    if (name === "replace_files") {
      ...
      for (const rel of await walk(selection.root.path, args.path || ".", {
        ...
        try {
          const document = await readTextDocument(path, args.input_encoding || "auto"); regex.lastIndex = 0;
          const matches = [...document.text.matchAll(regex)].length; regex.lastIndex = 0;
          if (matches) changes.push({ path, before: document.bytes,
            output: encodeTextDocument(document.text.replace(regex, String(args.replacement)), document, args), matches });
        } catch {}
      ...
      if (!dryRun) {
        ...
        catch (e) { for (const change of changes) await Deno.writeFile(change.path, change.before).catch(() => {}); throw e; }
      }
      return { server_opaque: selection.ws.id, root: selection.root.name, dry_run: dryRun,
        files: changes.map(change => ({ path: relative(selection.root.path, change.path).replaceAll("\\", "/"),
          replacements: change.matches, encoding: change.output.encoding, bom: change.output.bom,
          line_endings: change.output.line_endings })) };
    }
    if (name === "list_files") {
      const selection = selectedRoot(p, ownerKey, args);
      return { server_opaque: selection.ws.id, root: selection.root.name,
    ...
    if (name === "publish_file") {
      ...
      const result = await publishPath(target.path, {
        ...
        return_mode: args.return_mode || "link",
        allowed_root: target.root.path,
      });
      return { server_opaque: target.ws.id, root: target.root.name, path: target.display, ...result };
    }
    if (name === "list_commands") return await commandCatalog({ ...args, admin: false, include_missing: false });
    if (name === "js") {
      const { selection, key, kernel } = jsKernelContext(p, ownerKey, args);
      ...
      return {
        kernel_id: kernel.key, server_opaque: selection.ws.id,
    ...
    if (name === "js_reset") {
      const selection = selectedRoot(p, ownerKey, args);
      const key = jsKernelKey(p, ownerKey, selection.ws.id);
      return { reset: destroyJsKernel(key), kernel_id: key, server_opaque: selection.ws.id };
    }
    if (name === "exec" || name === "exec_start") {
      const rec = await startManagedProcess(p, ownerKey, args, name === "exec_start", execution);
    ...
    if (name === "exec_poll") return await pollManagedProcess(ownedProcess(args.process_id, ownerKey, args.server_opaque), args);
    if (name === "exec_write") {
      const rec = ownedProcess(args.process_id, ownerKey, args.server_opaque);
    ...
    if (name === "exec_list") return { processes: recentProcesses(ownerKey, args.server_opaque,
      args.include_completed !== false, Math.min(Number(args.limit || 50), 200)) };
    ...
    const rec = await startManagedProcess(p, ownerKey, spec, false, execution); await rec.done; return processView(rec);
  }
  ...
  function serverOpaqueEnvelope(opaqueValue, opaqueStatus = "active", retryRequired = false, message = "") {
    return {
      server_opaque: String(opaqueValue || ""), server_opaque_status: opaqueStatus,
      retry_required: !!retryRequired, message: String(message || ""),
    };
  }
  function serverOpaqueControlMessage(kind) {
    if (kind === "bootstrap") return "Generated a new server_opaque value. The requested operation was not executed. Repeat the same tool call with server_opaque set to this exact value, and keep passing it unchanged in every later tool call to this server.";
    if (kind === "expired") return "The supplied server_opaque value has expired. The requested operation was not executed. Omit server_opaque once to obtain a new value, then repeat the same tool call with the exact value returned by this server.";
    if (kind === "invalid") return "The supplied server_opaque value is invalid for this authenticated client. The requested operation was not executed. Retry with a previously issued valid value. If none is available, omit server_opaque once to obtain a new value. Never invent a replacement.";
    return "The server_opaque value is active. Keep passing this exact value unchanged in every later tool call to this server.";
  }
  function serverOpaqueControlToolResult(p, name, args, resolution) {
    const opaqueValue = resolution.record?.id || resolution.supplied_id || "";
    const envelope = serverOpaqueEnvelope(opaqueValue, resolution.server_opaque_status, true, serverOpaqueControlMessage(resolution.kind));
    const id = beginLog(p, name, args, opaqueValue), structuredContent = { execution_log_id: id, ...envelope };
    const toolResult = {
      content: [{ type: "text", text: JSON.stringify(structuredContent, null, 2) }],
      structuredContent, isError: false,
    };
    updateLog(id, {
      completed_at: Date.now(), duration_ms: 0,
      status: resolution.kind === "bootstrap" ? "completed" : "failed",
      ...
      error: resolution.kind === "bootstrap" ? "" : envelope.message,
    });
    indexLog(id);
    return toolResult;
  }
  ...
  async function callTool(p, name, args, callInfo) {
    const id = beginLog(p, name, args, callInfo.requestServerOpaque), started = Date.now();
    ...
    try {
      ...
      const envelope = serverOpaqueEnvelope(
        callInfo.requestServerOpaque, "active", false, serverOpaqueControlMessage("active"),
      );
...
${envelope.message}
server_opaque: ${envelope.server_opaque}`
...
[truncated; full output in log ${id}]` : full;
      ...
      const structuredContent = {
        execution_log_id: id, error: String(e?.message || e), ...envelope,
      };
      const toolResult = {
...
<button name=decision value=deny>Deny</button></form></div>`, 200, "text/html; charset=utf-8");
    ...
    const rejectParsedToolCall = (message, rule = "protocol validation") => {
      ...
      return rejectToolCall(p, toolName, x.params?.arguments || x.params || {}, message, String(x.params?.arguments?.server_opaque || ""));
    };
    ...
    const rootSummary = serverRoots(p).map(r => r.name).join(", ") || "none";
    const instructions = fullAccess
      ? `Tools operate inside the MrMCP server. Available named directories: ${rootSummary}. ` +
        "Each valid server_opaque value may select one named directory; when no root is configured, tool paths fall back to the folder containing mrmcp.js. " +
        `${SERVER_OPAQUE_RULE} ` +
        "For ordinary text changes, prefer edit_file/edit_files for exact replacements or replace_files for repeated literal/regex edits. These tools preserve encoding, BOM and line endings by default; use js/exec for editing only when the transformation genuinely requires programmatic parsing or cannot be expressed with the text tools. " +
        ...
        "Every authenticated client can invoke every published tool. Anonymous clients cannot invoke tools, and there are no per-tool user rules."
      ...
      if (x.method === "server/discover") {
        ...
        if (!fullAccess) {
          ...
          const logId = rejectToolCall(
            ...
            "Authentication required for tool execution", String(x.params?.arguments?.server_opaque || ""),
          );
          r.error = {
          ...
          const logId = rejectToolCall(
            ...
            "tools/call requires params.name", String(x.params?.arguments?.server_opaque || ""),
          );
          r.error = {
          ...
          const resolution = resolveServerOpaque(
            p, auth.ownerKey, toolArgs.server_opaque, observedProtocol,
          );
          const toolResult = resolution.kind === "active"
            ? await callTool(
                p, x.params.name, toolArgs,
                { ownerKey: auth.ownerKey, authKind: auth.kind,
                  requestServerOpaque: resolution.record.id },
              )
            : serverOpaqueControlToolResult(p, x.params.name, toolArgs, resolution);
          r.result = modernRequest
            ? {
                resultType: "complete",
                ...toolResult,
  ...
  const opaqueProjection = (serverId, roots = rootsProjection(serverId)) => all(`SELECT w.id,w.label,w.default_root selected_root,w.created_at,w.updated_at,w.last_active_at,w.protocol_version,
      (SELECT COUNT(*) FROM logs l WHERE l.server_opaque=w.id) tool_calls
    FROM server_opaques w WHERE w.server_id=? AND w.id LIKE 'opaque_%'
    ORDER BY w.last_active_at DESC,w.created_at DESC LIMIT 500`, serverId).map(opaque => ({
      ...opaque,
      kind: "server_opaque",
      display_label: opaque.id,
      server_opaque_status: serverOpaqueExpired(opaque) ? "expired" : "active",
      expires_at: Number(opaque.last_active_at || opaque.created_at || 0) + SERVER_OPAQUE_TTL_MS,
      effective_root: opaque.default_root || "mrmcp.js folder",
      fallback_root: !opaque.default_root,
      available_roots: roots.filter(root => root.enabled)
        .map(root => ({ name: root.name, selected: root.name === opaque.default_root })),
    }));
  ...
  const serverProjection = p => {
    ...
    return {
      ...
      protocol_versions: MCP_PROTOCOLS,
      opaque_ttl_days: Math.round(SERVER_OPAQUE_TTL_MS / 86400000),
    };
  };
  ...
  const state = (section = "all") => {
    ...
    if (["all", "sessions", "logs"].includes(section)) {
      roots ||= rootsProjection(p.id);
      result.opaque_values = opaqueProjection(p.id, roots);
    }
    if (["all", "dashboard"].includes(section)) {
      result.server = serverProjection(p);
      result.stats = {
        opaque_values: one("SELECT COUNT(*) n FROM server_opaques WHERE server_id=? AND id LIKE 'opaque_%'", p.id)?.n || 0,
...
<? if(section==="dashboard"){ ?><section id=dashboard class=page><div class=row><h2 class=grow>🏠 Dashboard</h2><span class=muted>One server · one endpoint · explicit opaque state handles</span></div><div id=cards class=grid></div><div class=dashboard-grid><div><h3>🌐 Server</h3><div id=endpoints></div></div><div><h3>🔒 TLS and connectivity</h3><div id=tlsStatus></div></div></div></section>
<? } else if(section==="sessions"){ ?><section id=sessions class=page><div class=row><h2 class=grow>💬 Sessions</h2><span class=muted>Live updates</span></div><p class=muted>The MCP transport is stateless. MrMCP presents server-issued opaque values as sessions only in this administration UI. A value may select a named root; without one it uses the folder beside <code>mrmcp.js</code>.</p><div id=opaqueList></div></section>
<? } else if(section==="roots"){ ?><section id=roots class=page><div class=row><h2 class=grow>📁 Roots</h2><button class=primary data-action=new-root>➕ Add root</button></div><p class=muted>Register a logical name and an existing directory path. Tool paths are relative to the root selected for the supplied <code>server_opaque</code> value. Values without an explicit selection use the folder containing <code>mrmcp.js</code>.</p><div id=rootList></div></section>
...
<? } else if(section==="settings"){ ?><section id=settings class=page><h2>⚙️ Settings</h2><div class=grid><div class=card><h3>🌐 Fixed listeners</h3><p><b>HTTP</b> <code>0.0.0.0:80</code> · ACME HTTP-01 only</p><p><b>HTTPS</b> <code>0.0.0.0:443</code> · MCP, OAuth and metadata</p><p><b>GUI</b> <code>http://127.0.0.1:${GUI_PORT}</code> · embedded WebView / browser</p><label>Public IPv4</label><div class=row><input id=publicIp readonly class=grow value="<?= settings.public_ip||'' ?>"><button data-action=detect-ip>🔎 Detect</button></div><label>Public base URL override</label><input id=externalUrl class=grow placeholder="https://mcp.example.com" value="<?= settings.external_url||'' ?>"><label>Public IPv4 lookup URLs (one per line)</label><textarea id=publicIpUrls><?= (settings.public_ip_urls||[]).join("\\n") ?></textarea><label>Automatic DNS suffix</label><input id=sslipSuffix placeholder="sslip.io" value="<?= settings.sslip_suffix||'sslip.io' ?>"><label>ACME directory URL</label><input id=acmeDirectoryUrl class=grow value="<?= settings.acme_directory_url||'' ?>"></div><div class=card><h3>🔒 Certificate</h3><label>Let's Encrypt email</label><input id=tlsEmail type=email value="<?= settings.tls_email||'' ?>"><div class=row><button data-action=issue-cert>🛡️ Check / request certificate</button></div><p class=muted>A valid certificate already present in .mrmcp is reused. Requests occur only when renewal is due and backoff permits them.</p></div><div class=card><h3>🖥️ Process environment</h3><label><input id=inheritSystemPath type=checkbox<?= settings.inherit_system_path?" checked":"" ?>> Include the system PATH in spawned processes and commands</label><p class=muted>When disabled, the child PATH contains only <code>.mrmcp/bin</code>. Other environment variables remain available.</p></div></div><p><button class=primary data-action=save-settings>💾 Save settings</button></p><pre id=settingsInfo></pre></section><? } ?>`,
    ...
    opaque: `<? const d=it.data||{},values=d.values||[]; ?><? if (!values.length) { ?><p class=muted>No sessions have been issued yet.</p><? } else { ?><table><tr><th>Session ID</th><th>State / protocol</th><th>Root</th><th>Activity</th><th>Tool calls</th><th></th></tr><? values.forEach(v=>{ ?><tr><td><code class=opaque-id><?= v.id ?></code></td><td class=nowrap><b class="<?= v.server_opaque_status === 'active' ? 'ok' : 'failed' ?>"><?= v.server_opaque_status === "active" ? "🟢 active" : "⌛ expired" ?></b><br><code><?= v.protocol_version||"unknown" ?></code></td><td><select data-action=opaque-root data-id="<?= v.id ?>"><option value=""<?= v.fallback_root?" selected":"" ?>>Fallback: mrmcp.js folder</option><? (v.available_roots||[]).forEach(r=>{ ?><option value="<?= r.name ?>"<?= r.selected?" selected":"" ?>><?= r.name ?></option><? }) ?></select><div class=muted><?= v.effective_root ?></div></td><td class=opaque-dates><div><span class=muted>Created</span> <?= it.logdt(v.created_at) ?></div><div><span class=muted>Updated</span> <?= it.logdt(v.updated_at) ?></div><div><span class=muted>Active</span> <?= it.logdt(v.last_active_at) ?></div><div><span class=muted>Expires</span> <?= it.logdt(v.expires_at) ?></div></td><td><?= v.tool_calls||0 ?></td><td><button class=danger data-action=delete-opaque data-id="<?= v.id ?>">🗑️ Delete</button></td></tr><? }) ?></table><? } ?>`,
  ...
  async function buildUiRenderModel() {
    ...
    viewState.opaqueValues = projection.opaque_values || [];
    ...
    if (section === "commands") {
      ...
      const query = new URLSearchParams({
        q: current.query, opaque: current.opaque, status: current.status,
  ...
  async function renderUiDocument() {
    ...
    if (section === "dashboard") {
      view = fillUiMount(view, "cards", await renderEtaFragment("cards", {
        sessions: projection.stats?.opaque_values || 0,
      ...
      view = fillUiMount(view, "opaqueList", await renderEtaFragment("opaque", { values: projection.opaque_values || [] }));
  ...
  async function uiRunConfirmedAction(dialog) {
    ...
    switch (dialog?.confirmAction) {
      case "delete-root":
        ...
        break;
      case "delete-opaque":
        await uiInternalApi("/api/opaque/delete", { method: "POST", body: { id: String(data.id || "") } });
        break;
      case "delete-command":
  ...
  async function handleUiAction(event) {
    ...
    switch (action) {
      ...
      case "delete-root":
        ...
        return;
      case "delete-opaque":
        uiConfirm("Delete session", "Delete this server-issued opaque state handle and its UI association?", "delete-opaque", { id: data.id });
        return;
      case "new-command":
    ...
    try {
      switch (event.type) {
        ...
        case "change": {
          ...
          if (id === "logOpaque") { uiState.logs.opaque = String(event.value || ""); uiState.logs.page = 1; }
          ...
          else if (event.dataset?.action === "opaque-root")
            await uiInternalApi("/api/opaque/select", { method: "POST", body: { id: event.dataset.id, root: event.value } });
  ...
  async function guiApi(req, u) {
    ...
    if (u.pathname === "/api/roots/save" && req.method === "POST") {
      const x = await bodyJson(req), p = serverConfig(), name = String(x.name || "").trim(),
        path = await Deno.realPath(String(x.path || ""));
      ...
      if (x.id) run("UPDATE roots SET name=?,path=?,enabled=? WHERE id=? AND server_id=?",
        name, path, +!!x.enabled, x.id, p.id);
      else run("INSERT INTO roots(server_id,name,path,enabled,created_at) VALUES(?,?,?,?,?)",
        p.id, name, path, +!!x.enabled, Date.now());
      return json({ ok: true });
    }
    if (u.pathname === "/api/roots/delete" && req.method === "POST") {
      const x = await bodyJson(req), p = serverConfig(), root = one(
        "SELECT * FROM roots WHERE id=? AND server_id=?", x.id, p.id,
      );
      if (!root) return json({ error: "Root not found" }, 404);
      run("UPDATE server_opaques SET pinned_roots_json='[]',default_root='',updated_at=? WHERE server_id=? AND default_root=?",
        Date.now(), p.id, root.name);
      run("DELETE FROM roots WHERE id=?", root.id);
      return json({ ok: true });
    }
    if (u.pathname === "/api/opaque/select" && req.method === "POST") {
      const x = await bodyJson(req), p = serverConfig(), ws = one(
        "SELECT * FROM server_opaques WHERE id=? AND server_id=?", String(x.id || ""), p.id,
      );
      if (!ws) return json({ error: "server_opaque value not found" }, 404);
      return json({ ok: true, server_opaque: selectWorkspaceRoot(p, ws, x.root) });
    }
    if (u.pathname === "/api/opaque/delete" && req.method === "POST") {
      const x = await bodyJson(req), p = serverConfig(), ws = one(
        "SELECT * FROM server_opaques WHERE id=? AND server_id=?", String(x.id || ""), p.id,
      );
      if (!ws) return json({ error: "server_opaque value not found" }, 404);
      run("DELETE FROM server_opaques WHERE id=?", ws.id);
      return json({ ok: true });
    }
    ...
    if (u.pathname === "/api/logs" && req.method === "GET") {
      const q = (u.searchParams.get("q") || "").trim(), opaque = u.searchParams.get("opaque") || "";
      const status = u.searchParams.get("status") || "";
      ...
      if (q && fts) {
        try {
          total = one(`SELECT COUNT(*) n FROM logs_fts f JOIN logs l ON l.id=CAST(f.log_id AS INTEGER)
            WHERE logs_fts MATCH ? AND (?='' OR l.server_opaque=?) AND (?='' OR l.status=?)`,
            q, opaque, opaque, status, status)?.n || 0;
          rows = all(`SELECT l.id,l.started_at,l.completed_at,l.server_opaque,l.tool,l.status,l.duration_ms
            FROM logs_fts f JOIN logs l ON l.id=CAST(f.log_id AS INTEGER)
            WHERE logs_fts MATCH ? AND (?='' OR l.server_opaque=?) AND (?='' OR l.status=?)
            ORDER BY l.started_at DESC LIMIT ? OFFSET ?`,
            q, opaque, opaque, status, status, pageSize, offset);
        } catch {}
      }
      if (!rows) {
        const like = `%${q}%`;
        total = one(`SELECT COUNT(*) n FROM logs WHERE (?='' OR server_opaque=?) AND (?='' OR status=?)
          AND (?='' OR server_opaque||tool||input_json||result_json||resolved_json||stdout||stderr||error LIKE ?)`,
          opaque, opaque, status, status, q, like)?.n || 0;
        rows = all(`SELECT id,started_at,completed_at,server_opaque,tool,status,duration_ms FROM logs
          WHERE (?='' OR server_opaque=?) AND (?='' OR status=?)
          AND (?='' OR server_opaque||tool||input_json||result_json||resolved_json||stdout||stderr||error LIKE ?)
          ORDER BY started_at DESC LIMIT ? OFFSET ?`,
          opaque, opaque, status, status, q, like, pageSize, offset);
      }
      const opaqueLabels = new Map(all("SELECT id FROM server_opaques")
      ...
      rows = rows.map(row => {
        ...
        return { ...row, opaque_label: opaqueLabels.get(row.server_opaque) || row.server_opaque || "unassigned",
    ...
    if (lm && req.method === "GET") return json(
      one(`SELECT id,started_at,completed_at,server_id,server_name,tool,status,input_json,resolved_json,
        stdout,stderr,error,result_json,duration_ms,server_opaque FROM logs WHERE id=?`, Number(lm[1])) || { error: "Not found" }
    );
