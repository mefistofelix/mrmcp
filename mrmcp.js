/*
MrMCP 0.10.72 — Deno-owned event-driven UI and stateless MCP server with explicit context capabilities and an embedded WebView desktop window.
...
async function backend() {
  ...
  const HEADER_ACTIVE_MS = 5 * 60 * 1000;
  ...
  function headerActivityProjection(p) {
    ...
    const toolCalls = one(`SELECT COUNT(*) total, SUM(status='failed') errors FROM logs WHERE server_id=?`, p.id) || {};
    return {
      ...
      active_window_minutes: HEADER_ACTIVE_MS / 60000,
    };
  }
...
(function () {
  ...
  function serverTools(p, fullAccess = true) {
    ...
    const execInput = {
      ...
      separate_streams: {
        ...
        description: "Also return stdout and stderr separately. By default process tools return one combined output stream in the order MrMCP observes data arriving from the two process pipes.",
      },
      ...contextInput,
    };
    ...
    const defs = {
      ...
      exec: [
        "Run a foreground command. Pass args verbatim and in order; consult the program's --help when CLI syntax is uncertain. Process output is normalized to readable plain text before it is buffered or stored. Read output together with status and exit_code to understand what happened; set separate_streams=true only when separate stdout/stderr are useful. Do not invoke shell commands, uv or Python to read, list, search, edit or replace files when the structured MrMCP tools cover the operation.",
  ...
  function rejectToolCall(p, tool, args, message, contextHandle = "") {
    ...
    indexLog(id);
    emitUiChange(["state"], "tool-call-rejected");
    return id;
  }
  ...
  const OAUTH_PAGE_TEMPLATE = `<? const d=it.data||{}; ?><!doctype html><html lang=en><head><meta charset=utf-8><meta name=viewport content="width=device-width,initial-scale=1"><meta name=color-scheme content=light><title><?= d.title ?> · MrMCP</title>
<style>
:root{color-scheme:light;--bg:#f3f6fb;--card:#fff;--text:#172033;--muted:#66758b;--line:#dce4ef;--brand:#5b54e8;--brand-soft:#eeedff;--approve:#15956f;--approve-hover:#10785b;--deny:#e95555;--deny-hover:#ce4141;--error:#c43f4d;--error-bg:#fff3f4;--error-line:#f1c5ca;--shadow:0 28px 80px rgba(25,38,65,.16)}
*{box-sizing:border-box}html,body{min-height:100%}body{margin:0;font:15px/1.5 system-ui,-apple-system,"Segoe UI",sans-serif;color:var(--text);background:radial-gradient(circle at 50% -12%,rgba(91,84,232,.18),transparent 38%),linear-gradient(180deg,#fafbff 0%,var(--bg) 100%)}
main{min-height:100vh;display:grid;place-items:center;padding:30px 18px}.card{width:min(100%,620px);background:var(--card);border:1px solid var(--line);border-radius:26px;box-shadow:var(--shadow);padding:36px}.brand{text-align:center}.logo-shell{width:92px;height:92px;margin:0 auto 16px;padding:8px;display:grid;place-items:center;border:1px solid #dedcff;border-radius:24px;background:linear-gradient(145deg,#fff,var(--brand-soft));box-shadow:0 10px 28px rgba(91,84,232,.15)}.logo{display:block;width:100%;height:100%;object-fit:contain;border-radius:17px}.eyebrow{display:inline-flex;align-items:center;padding:5px 10px;border-radius:999px;color:#5149cf;background:var(--brand-soft);font-size:12px;font-weight:750;letter-spacing:.035em;text-transform:uppercase;margin-bottom:10px}h1{font-size:30px;line-height:1.12;letter-spacing:-.02em;margin:0 0 10px}.subtitle{max-width:500px;margin:0 auto;color:var(--muted);font-size:16px}.subtitle strong{color:var(--text)}
.details{display:grid;grid-template-columns:1fr 1fr;gap:11px;margin:28px 0 16px}.detail{min-width:0;padding:14px 16px;border:1px solid var(--line);border-radius:16px;background:#fafcff}.detail.wide{grid-column:1/-1}.label{display:block;color:var(--muted);font-size:11px;font-weight:750;letter-spacing:.055em;text-transform:uppercase;margin-bottom:5px}.value{font-weight:700;overflow-wrap:anywhere}.value code{font:13px/1.45 ui-monospace,SFMono-Regular,Consolas,monospace;color:#344054}.notice{display:flex;gap:11px;align-items:flex-start;padding:14px 16px;border-radius:16px;background:#f0fbf7;border:1px solid #bce8d9;color:#286451;margin:16px 0 26px}.notice strong{color:#17785b}.notice.error{background:var(--error-bg);border-color:var(--error-line);color:#7f3640}.notice.error strong{color:var(--error)}.notice-icon{font-size:17px;line-height:1.35}.actions{display:flex;justify-content:center;gap:12px}.btn{appearance:none;border:0;border-radius:14px;min-width:190px;padding:14px 20px;font:750 15px system-ui,-apple-system,"Segoe UI",sans-serif;cursor:pointer;transition:transform .12s ease,background .12s ease,box-shadow .12s ease}.btn:hover{transform:translateY(-1px)}.approve{color:#fff;background:var(--approve);box-shadow:0 9px 22px rgba(21,149,111,.24)}.approve:hover{background:var(--approve-hover)}.deny{color:#fff;background:var(--deny);box-shadow:0 9px 22px rgba(233,85,85,.18)}.deny:hover{background:var(--deny-hover)}.foot{text-align:center;color:#8a96a8;font-size:12px;margin:22px 0 0}.error-card{border-color:#ecd2d6}
@media(max-width:560px){main{padding:18px 12px}.card{padding:28px 20px;border-radius:21px}.logo-shell{width:80px;height:80px;border-radius:21px}h1{font-size:26px}.details{grid-template-columns:1fr}.detail.wide{grid-column:auto}.actions{flex-direction:column}.btn{width:100%;min-width:0}}
...
<? if(d.error){ ?><div class="notice error"><span class=notice-icon>⚠️</span><div><strong><?= d.error ?></strong><br>Return to your client and start the connection again.</div></div><? } else { ?><div class=details><div class=detail><span class=label>Client</span><div class=value><?= d.client_name ?></div></div><div class=detail><span class=label>Requested access</span><div class=value><?= d.scope ?></div></div><div class="detail wide"><span class=label>Server resource</span><div class=value><code><?= d.resource ?></code></div></div><div class="detail wide"><span class=label>Return destination</span><div class=value><?= d.redirect_host ?></div></div></div><div class=notice><span class=notice-icon>🔒</span><div><strong>Only authorize clients you trust.</strong><br>The client will receive access to this MrMCP server. You can revoke it later from OAuth Clients. This request expires in 5 minutes.</div></div><form method=post action=/oauth/authorize><? (d.fields||[]).forEach(([name,value])=>{ ?><input type=hidden name="<?= name ?>" value="<?= value ?>"><? }) ?><div class=actions><button class="btn approve" name=decision value=approve>Authorize Access</button><button class="btn deny" name=decision value=deny>Cancel</button></div></form><? } ?><p class=foot>Secure OAuth authorization · MrMCP</p></section></main></body></html>`;
...
<? } else if(section==="help"){ ?><section id=help class=page><h2>❓ Help</h2><div class=card><h3>Connect ChatGPT Web</h3><ol><li>Make sure the Dashboard shows a trusted HTTPS certificate. ChatGPT needs a remote HTTPS MCP endpoint; use <code><?= settings.external_base_url ? settings.external_base_url + "/mcp" : "https://your-host/mcp" ?></code>.</li><li>In ChatGPT Web, enable Developer mode. In managed workspaces the current path is <b>Workspace settings → Permissions &amp; Roles → Connected Data Developer mode / Create custom MCP connectors</b>. Authorized users may also find the toggle under <b>Settings → Apps → Advanced Settings</b>.</li><li>Create a custom app from <b>Workspace settings → Apps → Create</b> or <b>Settings → Apps → Create</b>, enter the MrMCP endpoint, choose the offered authentication method, then select <b>Scan Tools</b>.</li><li>If OAuth is enabled in MrMCP, complete the authorization prompt. After the tool scan completes, create the app and select it from a new ChatGPT conversation.</li></ol></div><div class=card><h3>Authentication</h3><p>For ChatGPT, OAuth is the preferred MrMCP setup because ChatGPT can discover the authorization metadata, complete consent, and keep refresh-token connectivity. MrMCP also supports Basic authentication for MCP clients that offer it. Authentication grants access to the server; the <code>context_handle</code> selects persistent context state after authentication.</p></div><div class=card><h3>Write Access</h3><p>MrMCP does not maintain a separate read/write allowlist: every authenticated client receives every published tool. ChatGPT controls whether write/modify actions are usable through the app's permissions and action controls. As of this build, OpenAI documents full MCP write/modify support for Business, Enterprise and Edu; Pro custom MCP access is limited to read/fetch, and availability may change. Test write tools in Developer mode first. Where available, use <b>Workspace settings → Apps → Configure Actions / Action control</b> to enable the required actions. ChatGPT may still ask for confirmation before a write.</p></div><div class=card><h3>Using MrMCP in a Chat</h3><ol><li>Start a new chat and select the MrMCP app from the tools/apps menu.</li><li>On first use, ChatGPT should call <code>create_context</code>; later calls should reuse the returned <code>context_handle</code>.</li><li>Choose the working root for that Session in the MrMCP <b>Roots</b> page by dragging the Session into the desired root.</li><li>If you change ChatGPT model or thinking level, the MCP context may be recreated even inside the same conversation. Check the Sessions page if continuity matters.</li></ol><p class=muted>ChatGPT UI labels and plan availability can change. Current OpenAI references: <a href="https://help.openai.com/en/articles/12584461-developer-mode-and-mcp-apps-in-chatgpt" target=_blank rel=noopener>Developer mode and MCP apps in ChatGPT</a> · <a href="https://help.openai.com/en/articles/11487775-connectors-in-chatgpt" target=_blank rel=noopener>Apps in ChatGPT</a>.</p></div></section><? } ?>`,
    ...
    status: `<? const d=it.data||{},s=d.settings||{},a=d.activity||{},bad=!!s.mcp_listen_error,warn=!!s.listener_fallback,recent=a.recent_sessions||[],inFlight=a.tool_calls_in_flight||0,errors=a.tool_calls_errors||0; ?><span class="status-group <?= d.live!=="connected"?(d.live==="reconnecting"?"pending":"failed"):(bad?"failed":(warn?"pending":"ok")) ?>"><?= d.live!=="connected"?(d.live==="reconnecting"?"🟡 reconnecting":"🔴 offline"):(bad?"🔴 listener error":(warn?"🟡 fallback":"🟢 live")) ?></span><span class=status-group title="GUI / HTTP / HTTPS effective listener ports">🔌 <span class=status-ports><?= s.gui_port||"?" ?>/<?= s.mcp_http_active?s.mcp_http_port:"off" ?>/<?= s.mcp_https_active?s.mcp_https_port:"off" ?></span><? if(warn){ ?> <span class=pending>⚠</span><? } ?></span><span class=status-group title="Sessions with a Tool Call in the last <?= a.active_window_minutes||5 ?> minutes">💬 <span class="<?= a.active_sessions?'ok':'muted' ?>"><?= a.active_sessions||0 ?> active</span><? if(recent.length){ ?> · <span class=status-sessions><? recent.forEach((x,i)=>{ ?><?= i?" ":"" ?>#<?= x.id ?>(<?= x.tool_calls ?>)<? }) ?></span><? } ?></span><span class=status-group title="Tool Calls in flight / total recorded / failed">🛠️ <span class="<?= inFlight?'pending':'muted' ?>"><?= inFlight ?> in flight</span> · <span class=status-total><?= a.tool_calls_total||0 ?> total</span> · <span class="<?= errors?'failed':'muted' ?>"><?= errors ?> errors</span></span>`,
    ...
    oauth: `<table><tr><th>Client</th><th>Created</th><th>Sessions</th><th>Access Tokens</th><th>Refresh Tokens</th><th></th></tr><? (it.data || []).forEach(c => { ?><tr><td><b><?= c.name ?></b><div><code><?= c.client_id ?></code></div></td><td class=context-dates><?= it.logdt(c.created_at) ?></td><td class=context-dates><div><b><?= c.session_count||0 ?></b> <button class=small data-action=oauth-sessions data-id="<?= c.client_id ?>">💬 View Sessions</button></div><div><span class=muted>First created</span> <?= c.first_session_at ? it.logdt(c.first_session_at) : "—" ?></div><div><span class=muted>Last created</span> <?= c.last_session_at ? it.logdt(c.last_session_at) : "—" ?></div></td><td class=context-dates><div><b><?= c.token_count||0 ?></b></div><div><span class=muted>Last issued</span> <?= c.last_token_at ? it.logdt(c.last_token_at) : "—" ?></div></td><td class=context-dates><div><b><?= c.refresh_token_count||0 ?></b></div><div><span class=muted>Last used</span> <?= c.last_refresh_at ? it.logdt(c.last_refresh_at) : "—" ?></div></td><td><button class=danger data-action=revoke-client data-id="<?= c.client_id ?>">🚫 Revoke</button></td></tr><? }) ?></table>`,
