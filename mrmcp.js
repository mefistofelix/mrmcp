...
async function backend() {
  ...
  const EXEC_PREVIEW_ARG_LIMIT = 6, EXEC_PREVIEW_ARG_CHARS = 48, EXEC_PREVIEW_TOTAL_CHARS = 180;
  const compactPreviewValue = (value, limit = EXEC_PREVIEW_ARG_CHARS) => {
  ...
  const compactShellCommand = value => {
    ...
    return text.length > EXEC_PREVIEW_TOTAL_CHARS ? `${text.slice(0, EXEC_PREVIEW_TOTAL_CHARS - 1)}…` : text;
  };
  const compactExecCommand = (p, tool, args = {}) => {
    ...
    const argv = Array.isArray(spec.args) ? spec.args : [], shown = argv.slice(0, EXEC_PREVIEW_ARG_LIMIT).map(compactExecArg);
    const omitted = Math.max(0, argv.length - shown.length), suffix = omitted ? ` … +${omitted} args` : "";
    let line = [compactProgramName(spec.program), ...shown].filter(Boolean).join(" ");
    const available = Math.max(1, EXEC_PREVIEW_TOTAL_CHARS - suffix.length);
    if (line.length > available) line = `${line.slice(0, Math.max(1, available - 1))}…`;
    return line + suffix;
  };
  ...
  async function guiApi(req, u) {
    ...
    if (u.pathname === "/api/logs" && req.method === "GET") {
      ...
      rows = rows.map(row => {
        ...
        const input = parseJson(row.input_json || "{}", {}), command_preview = compactExecCommand(p, row.tool, input);
        const { input_json: _inputJson, ...summary } = row;
        return { ...summary, command_preview, killable: !!process || !!control?.cancel,
