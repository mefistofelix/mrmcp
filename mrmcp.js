/*
MrMCP 0.10.80 — Deno-owned event-driven UI and stateless MCP server with explicit context capabilities and an embedded WebView desktop window.
...
(function () {
  ...
  function rejectToolCall(p, tool, args, message, contextHandle = "", status = "failed", result = { error: message }) {
  ...
  async function mcpHandler(req, info, transport = "http") {
    ...
    const rejectParsedToolCall = (message, result = { error: message }) => {
      ...
      return rejectToolCall(
        ...
        String(x.params?.arguments?.context_handle || ""), "invalid", result,
      );
    };
        ...
        if (!fullAccess) {
          ...
          if (validationError) {
            ...
            rejectToolCall(
              ...
              String(rawToolArgs?.context_handle || ""), "invalid", { jsonrpc: "2.0", id: x.id, error: r.error },
            );
          } else {
